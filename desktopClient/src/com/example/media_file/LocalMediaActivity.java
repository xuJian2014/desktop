...
public class LocalMediaActivity extends Activity implements IReflashListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		listView.setInterface(LocalMediaActivity.this);
		...
		{
		case 0:
			sendMsgThread=new SendMsgThread(handler, LocalMediaActivity.this,JsonParse.Json2String(OptionEnum.Local_File_Videos.ordinal(), null));
			break;
		case 1:
			sendMsgThread=new SendMsgThread(handler, LocalMediaActivity.this,JsonParse.Json2String(OptionEnum.Local_File_Music.ordinal(), null));
			break;
		case 2:
			sendMsgThread=new SendMsgThread(handler, LocalMediaActivity.this,JsonParse.Json2String(OptionEnum.Local_File_Office.ordinal(), null));
			break;
		case 3:
			sendMsgThread=new SendMsgThread(handler, LocalMediaActivity.this,JsonParse.Json2String(OptionEnum.Local_File_Photos.ordinal(), null));
			break;
		default:
			break;
		}
			...
			switch(msg.what)
			{
				...
				case 5:
					if(mDialog!=null)
						mDialog.cancel();
					cleanListView();
					Toast.makeText(LocalMediaActivity.this, "对不起，连接家庭服务终端失败，请稍候重试", Toast.LENGTH_LONG).show();
					break;
				case 2://获取媒体库文件字符串
					...
					{
						Toast.makeText(LocalMediaActivity.this, "获取媒体库文件失败", Toast.LENGTH_LONG).show();
					}
					else
					{
				...
				case 3: //获取屏幕字符串
					...
					{
						Toast.makeText(LocalMediaActivity.this, "对不起,没有找到屏幕", Toast.LENGTH_LONG).show();
					}
					else
					{
				...
				case 4:
					...
					{
						Toast.makeText(LocalMediaActivity.this, "对不起，投影失败，请稍候重试...", Toast.LENGTH_LONG).show();
					}
					else
					{
						...
						if(errNum==ResponseMessageEnum.SUCCESS.ordinal())
						{
							...
							Toast.makeText(LocalMediaActivity.this, "投影成功！", Toast.LENGTH_LONG).show();
						}
						else if(errNum==ResponseMessageEnum.WAIT.ordinal())
						{
							Toast.makeText(LocalMediaActivity.this, "正在连接中，请稍后请重试...", Toast.LENGTH_LONG).show();
						}
						else if(errNum==ResponseMessageEnum.UNCONNECTED.ordinal())
						{
							Toast.makeText(LocalMediaActivity.this, "对不起，未能成功连接，请重试...", Toast.LENGTH_LONG).show();
						}
						else //error
						{
							//Toast.makeText(getActivity(), "投影"+adapter.getItem(info.position-1).getMediaName()+"失败！", Toast.LENGTH_LONG).show();
							Toast.makeText(LocalMediaActivity.this, "对不起，投影失败！", Toast.LENGTH_LONG).show();
						}
					}
					break;
				default:
	...
	private void binderListData(String[] str)
	{
		...
		adapter=new Media_List_Adapter(LocalMediaActivity.this, R.layout.content_list_item, mediaList);
	...
	{
		AlertDialog.Builder builder=new AlertDialog.Builder(LocalMediaActivity.this);
		...
		builder.setItems(screenList,new DialogInterface.OnClickListener()
		{
			...
			public void onClick(DialogInterface dialog, int which)
			{
				 if(info!=null)
				 {
					 mDialog = new ProgressDialog(LocalMediaActivity.this);  
			         mDialog.setTitle("投影");  
			         mDialog.setMessage("正在进行投影，请稍等...");  
			         mDialog.show();
			         ScreenResponseMsg msgAppScreen=new ScreenResponseMsg(handler, LocalMediaActivity.this, 
	...
	public void onCreateContextMenu(ContextMenu menu, View v,ContextMenuInfo menuInfo)
	{
		...
		MenuInflater inflater = LocalMediaActivity.this.getMenuInflater();
		...
		if(item.getMenuInfo() instanceof AdapterContextMenuInfo)
		{
			...
			switch (item.getItemId()) 
			{
				case R.id.screen: //投影屏幕
				     ...
				     mDialog.show();
					 SendMsgAppScreen sendMsgScreen=new SendMsgAppScreen(handler, LocalMediaActivity.this,JsonParse.Json2String(OptionEnum.DISPLAY.ordinal(), null));
	...
	{
		SendMsgThread sendMsgThread=new SendMsgThread(handler, LocalMediaActivity.this,JsonParse.Json2String(OptionEnum.FILE.ordinal(), null));
		Thread thread =new Thread(sendMsgThread);
		thread.start(); 
	}
	@Override
