...
public class HomeForScreenThread implements Runnable
{
	private final int TIME_OUT=10*1000;//最大响应时间，超时设置
	Socket socketClient=null;
	Handler handler=null;
	Activity activity;
	private String requestStr;
	public HomeForScreenThread(Handler handler,Activity activity,String requestStr) 
	{
		super();
	...
	public void run()
	{
		...
		try
		{
			...
			message.what=11;  //成功发送请求消息
			handler.sendMessage(message);
			
		}
		catch(ConnectException e)
		{
			message.what=9;// Connection refused
			handler.sendMessage(message);
		}
		catch (SocketException e) 
		{
			message.what=10;//Socket is closed
			handler.sendMessage(message);
		}
		catch (IOException e)
		{
			message.what=12;  // 连接发生错误
			handler.sendMessage(message);
		}
		finally
		{
