...
import com.example.touch.KeyListener;
...
public class GameActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run()
			{
				...
				try
				{
					...
					ConnectServer.getInstance().connect(DeviceConnection.getInstance().getIp());
					//ConnectServer.getInstance().connect("192.168.1.121");
					/*
					 * ConnectServer.getInstance().sendAction(new
					 * AuthentificationAction(password));
					 * AuthentificationResponseAction response =
					 * ConnectServer.getInstance().recvAction();
					 */
					isValid = true;
					Log.d("debug", "RUN");
				} catch (Exception e)
				{
					...
					Toast.makeText(GameActivity.this, "服务器连接失败", Toast.LENGTH_SHORT).show();
					finish();
				}
			}
		});
		...
		try
		{
			...
			{
				Log.d("debug", "vaild");
				Toast.makeText(GameActivity.this, "服务器连接成功", Toast.LENGTH_SHORT)
						.show();
	 ...
	 * super.onStart(); try { ConnectServer.getInstance().connect(ip);
	 * ConnectServer.getInstance().sendAction(new
	 * AuthentificationAction(password)); } catch (Exception e) {
	 * Toast.makeText(MainActivity.this, "start服务器连接失败",
